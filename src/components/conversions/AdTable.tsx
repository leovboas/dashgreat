...
import type { AdMetrics, AdSetMetrics } from '../../utils/computeMetrics'
...
export default function AdTable({
  ...
  const [activeTab, setActiveTab] = useState<'ads' | 'adsets'>('ads')
  ...
  if (byAd.length === 0 && byAdSet.length === 0) return null
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {adSetRows.map((r) => {
                  ...
                  return (
                    <tr key={r.adSet} className="hover:bg-blue-50 transition-colors">
